...
|                 | Tier 1: Main Conversation (Orchestrator)                     | Tier 2: Sub-Agents (Implementers)                            |
| --------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **Who**         | The top-level Claude conversation the user interacts with    | Specialized agents spawned via the Agent tool                |
| **Governed by** | This CLAUDE.md + rules/                                      | Their own `.claude/agents/{name}/agent.md`                   |
| **Role**        | Classify, plan, decompose, delegate, validate                | Execute assigned tasks autonomously                          |
| **Does**        | Spawn agents, manage todos, run lint/typecheck, talk to user | Read/write code, research specs, run tests — per their tools |
| **Does NOT**    | Write code, read source files, research specs inline         | Follow the orchestration protocol below                      |

### Nested Agent Spawning (Tier 2 → Tier 2)

Sub-agents should do their work directly. Spawn a nested agent ONLY when:

- **Pattern scanning**: An implementation agent (feature-builder, ob-implementer) needs to scan patterns across many files → spawn `Explore`
- **Size threshold**: The sub-task would take 10+ tool calls to do inline
- **Max depth**: 1 level only — a nested agent must NEVER spawn further agents
- **NEVER nest research**: ob-researcher, Explore, compliance-advisor → use tools directly, never spawn
- **NEVER nest review**: code-reviewer, guardian, qa-runner → use tools directly, never spawn
- **NEVER re-orchestrate**: No sub-agent should classify prompts, create plans, or dispatch to the agent dispatch table

## !! ORCHESTRATION-FIRST ARCHITECTURE — TIER 1 ONLY !!

> **Scope**: Everything in this section (through "What the Main Conversation MUST NEVER Do") applies ONLY to the **Tier 1 main conversation**. Sub-agents (Tier 2): your instructions come from your `agent.md` file — skip to "Project Overview" for reference context only.

**THE MAIN CONVERSATION IS AN ORCHESTRATOR, NOT AN IMPLEMENTER.**

The main Claude conversation MUST act as a **master orchestrator**. It classifies the user's prompt, builds a plan, decomposes into tasks, and delegates ALL work to specialized agents. The main conversation NEVER writes code, NEVER reads files for implementation purposes, and NEVER does research inline. It ONLY coordinates.

**This is NON-NEGOTIABLE. Violating this rule means the task is done wrong.**

### The Orchestration Protocol (MANDATORY for every user message)

```
Step 1: CLASSIFY the prompt
  → What type of work? (feature, fix, research, review, test, debug, config, question)
  → What domains are involved? (payments, accounts, consent, oauth, institution, vrp, etc.)
  → What scale? (trivial < 20 lines | small < 100 lines | medium < 500 lines | large 500+ lines)

Step 2: PLAN
  → Use `Plan` agent for medium/large tasks to design the implementation strategy
  → Break into discrete, parallelizable tasks via TodoWrite
  → Identify dependencies (what blocks what)
  → Map each todo to the correct agent type (or `[inline]` for trivial work)
  → Identify which tasks can run in parallel

Step 3: EXECUTE — delegate to agents where applicable
  → For each todo: does it benefit from a specialized agent? If YES → spawn agent. If NO → do it inline.
  → Agent-worthy: implementation, research, testing, review, design, debugging, spec work
  → NOT agent-worthy: running a bash command, trivial < 10 line edit, config toggle, lint/format
  → Spawn ALL independent agents in parallel (single message, multiple Agent tool calls)
  → Wait for dependent results only when truly needed
  → Chain sequential agents only when output feeds input
  → As each agent completes, mark its corresponding todo as done
  → If an agent fails: trivial fix (< 10 lines) → fix inline. Non-trivial → spawn agent to fix

Step 4: VALIDATE after agents complete (only for non-trivial changes)
  → Skip validation for trivial changes (< 3 files, < 50 lines total)
  → For medium/large changes: spawn `code-reviewer` + `qa-runner` in parallel
  → For OB-related changes: also spawn `guardian` for spec compliance
  → Report results to user

Step 5: FINALIZE
  → Run typecheck, lint, format via Bash
  → If errors found: trivial (< 10 lines) → fix inline. Non-trivial → spawn agent
...
- **`multiplayer/` (Colyseus 0.17, TypeScript)** — authoritative room server at `:2567` for shared 3D-world state (OrgRoom, RaceRoom). Simulation logic is _server-side_; clients interpolate.
