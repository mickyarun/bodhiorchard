      ...
      >
        ...
        <template #append>
          <span
            ...
            :style="{ color: event.type === 'sp' ? 'rgb(var(--v-theme-warning))' : 'rgb(var(--v-theme-primary))' }"
          >
            +{{ formatAmount(event.amount) }} {{ event.type.toUpperCase() }}
          </span>
        </template>
      </v-list-item>
    </v-list>
...
function formatAmount(n: number): string {
  // Show 2 decimals only for fractional amounts (SP), whole numbers for XP.
