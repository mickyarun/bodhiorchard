...
down_revision: str | None = "a3f7b2c1d4e5"
...
def upgrade() -> None:
    ...
    op.add_column(
        "bud_documents",
        sa.Column("qa_automation_cases", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    )
    op.add_column(
        "bud_documents",
        sa.Column("qa_manual_cases", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    )
