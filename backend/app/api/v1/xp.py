...
@router.get("/history", response_model=list[RewardEventRead])
async def get_xp_history(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> list[RewardEventRead]:
    """Return recent reward events (XP and SP) for the current user."""
    event_repo = RewardEventRepository(db, org_id=current_user.org_id)
...
async def claim_greeting_bonus(
    ...
    return {"awarded": True, "skill_points": new_sp, "sp_earned": 0.25}
