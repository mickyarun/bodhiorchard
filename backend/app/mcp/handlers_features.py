...
async def handle_get_features(
    ...
    """Keyword-search active features.

    Mirrors the frontend ``/features`` page's substring match on
    ``feature_title`` but widens it to also hit ``description`` so a
    multi-word LLM query (e.g. "payment links notes post-payment edit")
    can land on rows whose title alone wouldn't match. Tokens shorter
    than 2 chars are dropped.

    Required: ``query`` (non-empty). Optional: ``limit`` (default 10,
    max 50), ``offset`` (default 0). External LLMs paginate by calling
    with ``offset = offset + limit`` until ``has_more`` is false.

    Previously this used pgvector cosine search over ``Feature.embedding``;
    that path silently returned zero results when the corpus's
    embeddings were generated by a different fastembed model version
    than the live server (a real production gotcha after DB restores).
    Keyword works on any DB state.
    """
    ...
    # COUNT query. Trim back to ``limit`` before serializing.
