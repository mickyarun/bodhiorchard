...
class FeatureReadRepository:
    ...
    async def keyword_search(
        ...
        *,
        ...
        """Tokenised ILIKE search over ``feature_title`` + ``description``.

        Mirrors the frontend ``/features`` page semantics (which uses
        ``feature_title ILIKE %q%`` via ``list_with_links``) but widens
        the match to also cover the description so multi-word natural
        language queries from external LLMs land hits. The query is
        split on whitespace; tokens shorter than 2 chars are dropped to
        avoid matching every row. A row matches if ANY token matches
        title OR description.

        Used by the MCP ``get_features`` tool — chosen over
        ``semantic_search`` because (a) keyword behaviour matches the
        users' mental model from the UI and (b) keyword works on any
        DB state, while semantic depends on the corpus's embeddings
        having been generated by the SAME fastembed model the live
        server uses; a model-version drift across restores silently
        ...
        or_clauses = [
            or_(
                Feature.feature_title.ilike(f"%{tok}%"),
                Feature.description.ilike(f"%{tok}%"),
            )
            for tok in tokens
        ]
        stmt = (
            select(Feature)
            .where(Feature.org_id == self._org_id, or_(*or_clauses))
            .order_by(Feature.feature_title)
            .offset(offset)
            .limit(limit)
        )
