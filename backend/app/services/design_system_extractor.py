...
def _extraction_instructions(platform: Platform) -> str:
    ...
    if platform.kind == PlatformKind.WEB:
        return (
            ...
            "examples.\n"
            "7. **App Skeleton**: A minimal static HTML scaffold (≤80 "
            "lines) showing the app's primary navigation surface and the "
            "main content region as the user sees them today. Include "
            "top-level routes/pages as nav items with their labels and "
            "target paths (read the router config — e.g. `router/index.ts`, "
            "`App.vue`, `routes.tsx` — to source them; do not invent). "
            "Mark the content region with a clear placeholder comment "
            "(`<!-- PAGE CONTENT GOES HERE -->`). Use only tokens from "
            "sections 1–4 — no new colors or fonts. Wrap the entire "
            "skeleton block in `<!-- APP-SKELETON-BEGIN -->` … "
            "`<!-- APP-SKELETON-END -->` markers so downstream wireframe "
            "generators can extract it deterministically.\n\n"
