...
XPEvent). No external API calls — all data already lives in PostgreSQL.
...
async def _collect_xp_earned(
    ...
    result = await db.execute(
        select(XPEvent.user_id, func.sum(XPEvent.xp_amount).label("total"))
        .where(
            XPEvent.org_id == org_id,
            XPEvent.created_at >= since,
            XPEvent.created_at < until,
        )
