...
async def continue_triage(
    ...
    if session.session_type == "bug":
        await _run_bug_triage_agent(db, org, bot_token, session)
    else:
        await _run_triage_agent(db, org, bot_token, session)
...
async def handle_bug_approval(
    ...
    if not approver:
        await slack_client.chat_post_message(
            bot_token, channel,
            "⚠️ Could not resolve your user account. Please approve from a linked Slack account.",
            thread_ts=session.thread_ts,
        )
        return
    ...
    session.status = "bud_created"  # reuse the terminal status
...
async def _run_bug_triage_agent(
    db: AsyncSession,
    org: Organization,
    bot_token: str,
    session: TriageSession,
    ...
    Uses the same question/summary pattern as the BUD triage agent but
    with a bug-specific system prompt that asks for: title, description,
    steps to reproduce, severity, and module.
    """
    thread_messages = await slack_client.conversations_replies(
        bot_token, session.slack_channel, session.thread_ts
    )

    # Build a bug-specific prompt
